...
$(document).ready(function() {
    ...
    if (defaultLanguage == 'en') {
        ...
        crossWordLang.used_hint = 'Bạn không thể sử dụng gợi ý nhiều lần cho 1 ô chữ';
        ...
        if (isSameCoordinate(curTextBox, first)) {
            for (var i = 0; i < curWordDOM.uiSelected.length; i++) {
                if (!(curWordDOM.uiSelected[i]).attr('data-finish')) {
                    curWordDOM.uiSelected[i].focus().select();
                    return;
                }
            }
    ...
    */
    ...
    function insertWordNumber(wd) {
        ...
        wd.wordNumberDOM.click(function() {
            ...
            // Set clue
            if (wd.clue.indexOf('[') != -1) {
                wd.clue = highlightInBracket(wd.clue);
            }
            $('#info-board .clue .text').html(htmlEncode(wd.clue));
            $('#info-board .clue img').attr('src', resourceUrl + wd.imgUrl).show();
        });
