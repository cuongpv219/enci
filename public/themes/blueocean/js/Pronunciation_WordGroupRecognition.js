...
$(document).ready(function() {
    ...
    function inputChange(e) {
        if (e.results) {
            var precision = getPrecision(activityContent.card[curTabIndex].term, e.results[0].utterance, e.results[0].confidence);
    ...
    function activateTabAndButton(index) {
        ...
        $('#speak-word').text(activityContent.card[curTabIndex].term);
        $('#speak-word-ipa').text(htmlEncode(activityContent.card[curTabIndex].ipa));
    ...
    if (!is_chrome) {
        var errorMessageHtml = '<div id="error-message">This activity only works on Chrome</div>\n\
            <div class="global-submit-div"><div id="loadResult" class="global-button-green-1">' + multiLangSystem.button_finish + '</div></div>';
        $('#activity-container').html(errorMessageHtml);
        return;
    }

    count = activityContent.card.length; //number of sentences
    var resultArr = [];
    var statisticArr = [];
    var curTabIndex = 0;
    var buttonHtml = '';
    for (var i = 0; i < count; i++) {
        var arr = [];
        resultArr[i] = arr;
        statisticArr[i] = new Array(0, 0, 0, 0);
        buttonHtml += '<div id="sentence-button_' + i + '" data-order="' + i + '" class="inactive-button navigator-button unselected" data-active="0">' + (i + 1) + '</div>';
    }
    ...
    $('#speaker-wrapper').click(function() {
        playSound(resourceUrl + activityContent.card[curTabIndex].audio);
    });
    ...
    function drawPrecision(eventResults){
        var precision = getPrecision(activityContent.card[curTabIndex].term, eventResults.transcript, eventResults.confidence);
