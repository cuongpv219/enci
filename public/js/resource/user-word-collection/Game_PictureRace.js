        ...
        for(var i=0; i<index; i++) {
            if((curPos>300*i)&&(parseInt($('#picture_'+ranArr[i]).css('left').replace("px","")) < 1000 )  && ($('#picture_'+ranArr[i]).attr("value")==0) ) 
            {
                ...
                if(parseInt($('#picture_'+ranArr[i]).css('left').replace("px","") ) >= 1000) {
                    ...
                    $('.true-answer').html(activityContent.card[ranArr[i]].title);
...
$(document).ready(function() {
    ...
    $('#front-flat-menu-item-3').addClass('front-flat-menu-item-active').removeClass('front-flat-menu-item-inactive');
            ...
            for(var i=0; i<count; i++) {
                if((parseInt($('#picture_'+i).css('left').replace("px","") ) < 1000) && parseInt($('#picture_'+i).css('left').replace("px","") ) > -110)
                    if (checkTextTypedByLearner(text_typed_by_learner
                        ...
                        .replace(/^\s+|\s+$/g, "")
                        .toLowerCase()
                        , activityContent.card[i].title.toLowerCase())) {
    ...
    function displayActivity(){
        ...
        for(var i=0; i<count; i++) {
            ...
            var pictureHtml = "";
            if(pictures[i] != null) {
                pictureHtml += '<img class="picture" value="0" id="picture_'+i+'" src="'+
                pictures[i]+'"/>';
            }
            else {
                pictureHtml += '<div class="picture noimage" value="0" id="picture_'+i+'">'+activityContent.card[i].term+'</div>';
            }
